  ...
  def install_plugin name, params={}
    ...
    ruby_block "Install plugin: #{name}" do
      block do
        ...
        url     = params['url']     ? " -u #{params['url']}" : nil

        command = "#{node.elasticsearch[:env][:es_home]}/bin/plugin -i #{name}#{version}#{url}"
        Chef::Log.debug command

        raise "[!] Failed to install plugin" unless system command

        # Ensure proper permissions
        raise "[!] Failed to set permission" unless system "chown -R #{node.elasticsearch[:env][:es_user]}:#{node.elasticsearch[:env][:es_user]} #{node.elasticsearch[:env][:es_home]}/plugins/"
      end

      notifies :restart, 'service[elasticsearch]' unless node.elasticsearch[:skip_restart]

      not_if do
        Dir.entries("#{node.elasticsearch[:env][:es_home]}/plugins/").any? do |plugin|
          next if plugin =~ /^\./
          name.include? plugin
        end rescue false
      end
